//Author: S Middleton
//Date: 2021
//Purpose: EventDisplay Driving module
...
namespace mu2e
{
    ...
    class EventDisplay : public art::EDAnalyzer {
      public:
        ...
        explicit EventDisplay(const Parameters& conf);
  ...
  EventDisplay::EventDisplay(const Parameters& conf)  :
    art::EDAnalyzer(conf),
  ...
  void EventDisplay::signalAppStart()
  {
  ...
  void EventDisplay::beginJob(){
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : beginJob()] -- starting ..."<<std::endl;
      {
      std::unique_lock lock{m_};
      ...
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : beginJob()] -- starting wait on app start"<<std::endl;
      cv_.wait(lock);
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : beginJob()] -- app start signal received, starting eve init"<<std::endl;
      ...
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : beginJob()] -- starting wait on eve setup"<<std::endl;
      cv_.wait(lock);
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : beginJob()] -- eve setup apparently complete"<<std::endl;
      }
  }


  void EventDisplay::beginRun(const art::Run&){

  }

  void EventDisplay::printOpts(){
    std::cout<<"*********** REve Mu2e **************"
    <<" User Options: "
  ...
  template <class T, class S> void EventDisplay::FillAnyCollection(const art::Event& evt, std::vector<std::shared_ptr<REveMu2eDataProduct>>& list, std::tuple<std::vector<std::string>, std::vector<S>>& tuple){
  ...
  void EventDisplay::analyze(art::Event const& event){
      ...
      if((seqMode_) or ( runid_ == runn and subrunid_ == subrunn and eventid_ == eventn)){
        ...
        std::unique_lock lock{m_};
        if(diagLevel_ == 1) std::cout<<"[EventDisplay : analyze()] -- Fill collections "<<std::endl;
        //auto start1 = std::chrono::high_resolution_clock::now();
        ...
        if(diagLevel_ == 1) std::cout<<"[EventDisplay : analyze()] -- Event processing started "<<std::endl;
        XThreadTimer proc_timer([this]{ process_single_event(); });

        if(diagLevel_ == 1) std::cout<<"[EventDisplay : analyze()] -- transferring to TApplication thread "<<std::endl;
        cv_.wait(lock);
        if(diagLevel_ == 1) std::cout<<"[EventDisplay : analyze()] -- TApplication thread returning control "<<std::endl;
        if(diagLevel_ == 1) std::cout<<"[EventDisplay : analyze()] Ended Event "<<std::endl;
        seqMode_ = true;
    ...
    {
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : EndJob] Start "<<std::endl;
      application_.Terminate(0);

      if (appThread_.joinable()) {
        appThread_.join();
      }
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : EndJob] End "<<std::endl;
    }



  // Functions invoked by the threads explicitly created by this module
  void EventDisplay::run_application()
  {
  ...
  void EventDisplay::setup_eve()
  {
      ...
      std::cout<<"[EventDisplay::setup_eve] run in display is set to "<<eventMgr_->run<<std::endl;
  ...
  void EventDisplay::process_single_event()
    {
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : process_single_event] Start "<<std::endl;
      ...
      std::cout<<"[EventDisplay::process_single_event] display has run number set to "<<eventMgr_->run<<std::endl;
      std::cout<<"[EventDisplay::process_single_event] value in the text class "<<fText->get()<<std::endl;

      fGui->StampObjProps();

      if(diagLevel_ == 1) std::cout<<"[EventDisplay : process_single_event] -- extract event scene "<<std::endl;
      REX::REveElement* scene = eveMng_->GetEventScene();

      if(diagLevel_ == 1) std::cout<<"[EventDisplay : process_single_event] -- calls to data interface "<<std::endl;
      ...
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : process_single_event] -- cluster added to scene "<<std::endl;
      ...
      if(diagLevel_ == 1) std::cout<<"[EventDisplay : process_single_event] End "<<std::endl;
    }
  }
